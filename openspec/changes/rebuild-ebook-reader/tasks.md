...
- [ ] 8.1 建立 `ReaderSettings` 抽屜元件
- [ ] 8.2 實作排版切換（直排 ↔ 橫排），即時套用不重載書籍
- [ ] 8.3 實作字體大小調整，即時注入 CSS 到 iframe
- [ ] 8.4 實作主題切換（亮色/護眼/暗色），即時套用
- [ ] 8.5 實作簡繁轉換（opencc-js）
- [ ] 8.6 整合 `PUT /api/users/:userId/settings` 儲存設定
